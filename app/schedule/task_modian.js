...
class UpdateCache extends Subscription {
  ...
  async subscribe() {
    ...
    for (const iterator of order_list) {
      if (_.find(modian_user_list, { user_id: iterator.user_id, pay_time: iterator.pay_time })) {
        continue;
      }
      ...
      let donate_detail = await this.app.getModianDetail(form, this.app.config.modian_detail_url);
      donate_detail = donate_detail[0];

      const msg =
        `感谢 ${iterator.nickname} 刚刚在${donate_detail.pro_name}中支持了：${iterator.backer_money}元！ \n` +
        `${this.app.config.target_site} \n` +
